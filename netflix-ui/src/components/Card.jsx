...
export default React.memo(function Card({ index, movieData }) {
  ...
  if (likedMovies != undefined) {
    duplicate = likedMovies.some((movie) => movie.id === id);
  }
  // console.log(duplicate);

  // if (duplicate) setIsLiked(true);
  ...
  const addToList = async () => {
    try {
      ...
      if (likedMovies.length === 0) {
        dispatch(setlikedmovies([movieData]));
      } else dispatch(setlikedmovies([movieData, ...likedMovies]));
    } catch (error) {
      console.log(error);
    }
  };
            ...
            <div className="icons flex j-between">
              <div className="controls flex">
                ...
                {duplicate || isliked ? (
                  ...
                  <AiOutlinePlus title="WatchList" onClick={addToList} />
                )}
              </div>
              <div className="info">
